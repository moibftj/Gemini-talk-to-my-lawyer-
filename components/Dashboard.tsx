...
import { STATUS_STYLES, IconFilePlus } from '../constants';
...
export const Dashboard: React.FC<{ letters: LetterRequest[], onNewLetterClick: () => void }> = ({ letters, onNewLetterClick }) => {
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <ShimmerButton onClick={onNewLetterClick} className="shadow-2xl">
            <span className="whitespace-pre-wrap text-center text-sm font-medium leading-none tracking-tight text-white dark:from-white dark:to-slate-900/10 flex items-center gap-2">
      ...
      <CardContent className="p-0">
        <div className="border-t border-gray-200 dark:border-gray-800">
            {letters.length > 0 ? (
                letters.map(letter => <LetterRow key={letter.id} letter={letter} />)
            ) : (
